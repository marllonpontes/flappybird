...
public class jogo extends ApplicationAdapter {
	...
	private boolean passouCano = false;//Por padrão já recebe falso
	...
	BitmapFont textoInicioJogo;
	BitmapFont textoNomeCriadorDoJogo;
	...
	private void verificarEstadoDoJogo(){
		...
		if ( estadoJogo == 0 ){
		    ...
		    //Verificar se a pontuação é maior que o recorde
	...
	public void validarPontos(){
		...
		if ( posicaoCanoHorizontal < 50 - passaros[0].getWidth() ){//Passou do pássaro
			if ( !passouCano ){
				pontos++;
				passouCano = true;
				somPontuacao.play();
	...
	private void desenharTexturas(){
		...
		if (  estadoJogo == 0){
			textoInicioJogo.draw(batch, "Toque para começar", larguraDispositivo / 2 - 180 , alturaDispositivo / 2 );
			textoNomeCriadorDoJogo.draw(batch, "by: Marllon Pontes", larguraDispositivo / 3 - 10, alturaDispositivo / 8 - 10 );
		}
	...
	private void inicializarObjetos(){
		...
		textoInicioJogo = new BitmapFont();
		textoInicioJogo.setColor(Color.WHITE);
		textoInicioJogo.getData().setScale(3);

		textoNomeCriadorDoJogo = new BitmapFont();
		textoNomeCriadorDoJogo.setColor(Color.WHITE);
		textoNomeCriadorDoJogo.getData().setScale(2);
